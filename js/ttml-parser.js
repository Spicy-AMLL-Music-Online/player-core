...
* Updated to natively parse JSON lyric payloads.
*/
...
/**
* Parse a TTML string or JSON lyric payload into structured lyrics data.
* @param {string|object} input - Raw TTML XML string OR JSON payload containing syncedLyrics
* @returns {object} Parsed lyrics object with Type, Content, etc.
*/
export default function parseTTMLToLyrics(input) {
 let ttml = input;

 // Extract syncedLyrics if input is a JSON string or object
   ...
   if (trimmed.startsWith('{') && trimmed.endsWith('}')) {
     try {
       ...
       if (parsed && typeof parsed === 'object') {
         // Prioritize syncedLyrics over everything else
         ttml = parsed.syncedLyrics || parsed.ttml || trimmed;
       }
     } catch (e) {
       // Ignore JSON parse errors and continue trying to parse as raw TTML
     }
   }
 } else if (input !== null && typeof input === 'object') {
   ttml = input.syncedLyrics || input.ttml;
 }

 if (typeof ttml !== 'string' || !ttml.trim()) {
   throw new Error("Invalid input: Expected a valid TTML string or a JSON payload containing 'syncedLyrics'.");
 }
